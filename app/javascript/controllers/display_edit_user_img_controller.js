...
var img_list = []
// Connects to data-controller="display-edit-user-img"
  ...
  displayImage(){
      ...
      if (this.imgGetterTarget.files) {
        for (let i = 0; i < img_list.length; i++) {
          this.divDisplayingTarget.innerHTML += "<span class='relative mx-1 w-w-full' data-img-display-on-create-store-target='imgDiv'> <span class='cursor-pointer absolute top-1 left-2 z-10' data-img-display-on-create-store-target='deleteImage' data-action='click->img-display-on-create-store#deleteImg' data-img-display-on-create-store-id-param='"+i+"'> <i class='fal fa-times text-xl text-white'></i> </span> <img src="+URL.createObjectURL(img_list[i])+" class='h-96 w-96 rounded-sm object-cover border opacity-80 border-gray-300' /></span>"
        }
      }
  }
  deleteImg(items){
